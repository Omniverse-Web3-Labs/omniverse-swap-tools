...
async function init(chainName, tokenId) {
  ...
  omniverseContract = new ContractPromise(
    api,
    metadata,
    netConfig.omniverseContractAddress[tokenId]
  );
  return true;
}
...
let getRawData = (txData) => {
  ...
  console.log(
    'Buffer.from([fungible.op])',
    fungible,
    Buffer.from([fungible.op]),
    Buffer.from(fungible.ex_data),
    Buffer.from(
...
async function initialize(members) {
  let ret = await ink.sendTransaction(
    omniverseContract,
    'omniverse::setCoolingDown',
    sender,
    [netConfig.coolingDown]
  );
  if (!ret) {
    // Error
  }
...
async function sendTransaction(to, amount, op) {
  ...
  let txData = {
    ...
    chainId: chainId,
    initiateSc:
      '0x' + Buffer.from(omniverseContract.address.toJSON()).toString('hex'),
    from: publicKey,
...
async function omniverseBalanceOf(pk) {
  let amount = await ink.contractCall(
    omniverseContract,
    'fungibleToken::balanceOf',
    sender.address,
    [pk]
  );
  return amount;
...
(async function () {
  ...
  program
    ...
    .option('-ti, --tokenId <tokenId>', 'The omniverse token id')
    .parse(process.argv);
  ...
  if (program.opts().account) {
    ...
    await sendTransaction(program.opts().mint[1], program.opts().mint[2], MINT);
    ...
    let amount = await omniverseBalanceOf(account);
